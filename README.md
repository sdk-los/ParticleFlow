...
> **Важно:** при развёртывании в поддиректории (например, `https://user.github.io/effects-site/`) необходимо обновить поле `"start_url"` в [`manifest.json`](manifest.json) на `/effects-site/` (или соответствующую директорию), иначе PWA-установка может работать некорректно.
